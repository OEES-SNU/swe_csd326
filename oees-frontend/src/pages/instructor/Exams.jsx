...
export default function Exams() {
  ...
  const [selectedExamUnit, setSelectedExamUnit] = useState('')
  ...
  // Handles unit on exam list. Supports common field names.
  const getExamUnit = (ex) => ex.unit ?? ex.questionUnit ?? ex.unitName ?? ''
  const examUnits = [...new Set(exams.map(getExamUnit).filter(Boolean))]
  const filteredExams = selectedExamUnit
      ? exams.filter((ex) => getExamUnit(ex) === selectedExamUnit)
      : exams
  ...
  return (
      <>
        ...
        <div className="flex items-center gap-3 mb-5 flex-wrap">
          ...
          <select
              className="select max-w-xs"
              value={selectedExamUnit}
              onChange={(e) => setSelectedExamUnit(e.target.value)}
          >
              ...
              <table className="table">
                ...
                {filteredExams.map((ex) => (
                    <tr key={ex.id}>
                      ...
                      <td className="text-gray-500">{getExamUnit(ex) || '—'}</td>
                  ...
                  <div className="flex flex-1 min-h-0">
                    ...
                    <div className="flex-1 flex flex-col min-w-0 border-r border-gray-200">
                      ...
                      <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-4 flex-shrink-0">
                        ...
                        <select
                            className="select max-w-[140px] text-xs py-1.5"
                        ...
                        <select
                            className="select max-w-[160px] text-xs py-1.5"
                            value={filters.type}
                        ...
                        >
                          <option value="">All units</option>
                          {Array.from(new Set(allQuestions.map((q) => q.unit).filter(Boolean))).map((u) => (
